...
url = "https://www.newegg.com/p/pl?d=rx+9070"
webbrowser.open(url)

client = request(url)
# Opening the url
html = client.read()
# Grabbing the HTML into a parsed document
client.close()
# Closes the connection

# Turns into a parsed document
page_soup = soup(html, 'html.parser')
cells = page_soup.find_all("div", attrs={"class": "item-cell"})
# Calls find_all to retrieve the HTML code directly associated with the GPU we want to scrape

# Setting up the CSV to write to
with open('graphic_cards.csv', mode = 'w', newline='') as graphic_cards_file:
    file_writer = csv.writer(graphic_cards_file)
